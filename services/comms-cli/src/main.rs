...
#[derive(Parser, Debug)]
#[command(
    about = "Typed GraphQL client for the RADSAT communications service",
...
enum Command {
    ...
    Nmp {
        ...
        /// Optional NMP key override in decimal or `0x` hexadecimal form.
        /// When omitted, the service uses the key configured for this radio.
        key: Option<NmpKey>,
    ...
    /// Catch the common NPM/NMP transposition and provide a useful error.
...
enum NmpCommand {
    ...
    SetRoutingFromRs485 {
        /// Whether routing from RS-485 should be enabled (`true` or `false`).
        #[arg(action = clap::ArgAction::Set)]
        enabled: bool,
    },
    /// Set the operating frequency in Hz.
    SetFrequency {
        /// New operating frequency in hertz.
        frequency_hz: u32,
    },
    ...
    SetTxEnable {
        /// Whether RF transmission should be enabled (`true` or `false`).
        #[arg(action = clap::ArgAction::Set)]
        enabled: bool,
    },
    /// Set the HDLC preamble size (25 through 300).
    SetPreambleSize {
        /// HDLC preamble size (25 through 300).
        size: u16,
    },
    /// Select normal power (true) or micropower (false).
    SetNormalPower {
        /// `true` for normal power; `false` for micropower.
        #[arg(action = clap::ArgAction::Set)]
        normal_power: bool,
    },
    /// Set the ground-segment receive-to-transmit delay in milliseconds.
    SetGsRxTxDelay {
        /// Ground-segment RX-to-TX delay in milliseconds (50 through 1000).
        delay_ms: u16,
    },
    /// Enable or disable the ham-radio digipeater.
    SetDigipeaterEnable {
        /// Whether the digipeater should be enabled (`true` or `false`).
        #[arg(action = clap::ArgAction::Set)]
        enabled: bool,
    },
    /// Set the fixed six-byte AX.25 callsign.
    SetCallsign {
        /// Exactly six bytes of text, or encoded data with `--format hex`.
        callsign: String,
        /// Encoding of CALLSIGN.
        #[arg(long, value_enum, ignore_case = true, default_value = "text")]
        format: DataFormat,
    },
    /// Set the custom Morse message (up to 20 ASCII bytes).
    SetMorseCustomMessage {
        /// Custom Morse text (up to 20 ASCII bytes).
        message: String,
    },
    /// Set the fixed four-byte custom Morse identification.
    SetMorseCustomIdent {
        /// Exactly four bytes of text, or encoded data with `--format hex`.
        ident: String,
        /// Encoding of IDENT.
        #[arg(long, value_enum, ignore_case = true, default_value = "text")]
        format: DataFormat,
    ...
    SetUserKey {
        /// Replacement unsigned 32-bit user key.
        new_user_key: u32,
    },
    /// Replace the NMP superuser key.
    SetSuperuserKey {
        /// Replacement unsigned 32-bit superuser key.
        new_superuser_key: u32,
    },
    /// Replace the fixed five-byte ITU shutdown key.
    SetItuKey {
        /// Exactly five bytes of text, or encoded data with `--format hex`.
        itu_key: String,
        /// Encoding of ITU_KEY.
        #[arg(long, value_enum, ignore_case = true, default_value = "text")]
        format: DataFormat,
    ...
    SetFsoo {
        /// Whether transmission inhibit is enabled (`true` or `false`).
        #[arg(action = clap::ArgAction::Set)]
        inhibit: bool,
        /// Inhibit period in milliseconds; the usual value is 2040000.
        period_ms: u64,
    },
    ...
    SetRssiContributionRatio {
        /// New-sample contribution percentage (1 through 99).
        ratio: u8,
    },
    /// Set the communication watchdog reset period (12 through 96 hours).
    SetCheckCommResetPeriod {
        /// Communication watchdog period in hours (12 through 96).
        hours: u16,
    },
    /// Set the AX.25 status and Morse telemetry periods in milliseconds.
    SetSystemStatusAndMorsePeriod {
        /// AX.25 system-status period in milliseconds (5000 through 180000).
        ax25_system_status_period_ms: u32,
        /// Morse task period in milliseconds (60000 through 3672000).
...
fn run(cli: Cli) -> Result<(), String> {
    let request = match cli.command {
        ...
        Command::Npm { arguments: _ } => {
            return Err(
                "unknown command `npm`\n\n  DID YOU MEAN `nmp`?\n  NMP is the Needronix Management Protocol.\n  Run `comms-cli nmp --help` for its commands."
                    .to_string(),
            );
        }
    };
...
impl NmpCommand {
    fn graphql_request(self, role: RadioRole, key: Option<NmpKey>) -> GraphqlRequest {
        match self {
...
fn request<const N: usize>(
    ...
    if let Some(key) = key {
        variables.insert("key".to_string(), json!(key.0));
    }
...
impl FromStr for NmpKey {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        ...
        parsed
            .map(Self)
            .map_err(|_| format!("`{value}` is not a valid 32-bit decimal or hexadecimal key"))
    }
}
...
mod tests {
    ...
    #[test]
    fn accepts_omitted_decimal_and_hex_key_overrides() {
        let omitted = Cli::try_parse_from(["comms-cli", "nmp", "uplink", "get-link-type"]).unwrap();
        assert!(matches!(omitted.command, Command::Nmp { key: None, .. }));

        let hexadecimal =
            Cli::try_parse_from(["comms-cli", "nmp", "uplink", "0xFEDCBA98", "get-link-type"])
                .unwrap();
        assert!(matches!(
            hexadecimal.command,
            Command::Nmp {
                key: Some(NmpKey(0xFEDC_BA98)),
                ..
            }
        ));
    }
    ...
    #[test]
    fn rejects_npm_with_a_specific_nmp_suggestion() {
        let cli = Cli::try_parse_from(["comms-cli", "npm", "--help"]).unwrap();
        let error = run(cli).unwrap_err();
        assert!(error.contains("unknown command `npm`"));
        assert!(error.contains("DID YOU MEAN `nmp`?"));

        let mut command = Cli::command();
        let help = command.render_long_help().to_string();
        assert!(!help.contains("aliases: npm"));
    }

    #[test]
    fn parses_and_documents_boolean_arguments() {
        let cli = Cli::try_parse_from([
            "comms-cli",
            "nmp",
            "downlink",
        ...
        assert!(matches!(
            cli.command,
            Command::Nmp {
                command: NmpCommand::SetTxEnable { enabled: true },
                ..
            }
        ));

        let help =
            Cli::try_parse_from(["comms-cli", "nmp", "downlink", "123", "set-fsoo", "--help"])
                .unwrap_err();
        assert_eq!(help.kind(), ErrorKind::DisplayHelp);
        assert!(help.to_string().contains("<INHIBIT> <PERIOD_MS>"));
        assert!(help.to_string().contains("possible values: true, false"));
    }
    ...
    fn parses_routes_and_builds_graphql_variables() {
        ...
        .graphql_request(RadioRole::Uplink, Some(NmpKey(42)));
    ...
    fn uses_variables_for_string_and_enum_inputs() {
        ...
        .graphql_request(RadioRole::Downlink, Some(NmpKey(7)));
    ...
    fn every_cli_operation_validates_against_the_service_schema() {
        ...
        for command in commands {
            let request = command.graphql_request(RadioRole::Downlink, None);
