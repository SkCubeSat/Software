---
title: Default Testing Procedure
description: Local-first test workflow, then KubOS OBC integration testing when hardware is required.
---
...
```bash
./<service-name> -c <config_file.toml> &
```
...
We initially had the default baud rate of `115200` configured in UBOOT, but later changed it in a PR.

The baud rate we use now is **921600**.

So this might not be needed anymore. But in case you ever need to change the baud rate, use:
