...
<img src="/kubos/images/architecture_stack.png" />
</figure>
...
Information about how to develop and tie all of these components together for a particular mission can be found in the [mission development](../mission-dev/index) section.
...
Details about how to get started developing applications can be found in the [mission app dev guide](apps/app-guide).

If you prefer to learn by doing, check out our [tutorials](../tutorials/index) section.

For more information about what applications will need to be developed in order to accomplish all the required mission logic, check out our [../mission-dev/mission-needs](../mission-dev/mission-needs) doc.
...
All services expose their functionality via HTTP endpoints which accept [GraphQL](services/graphql) requests and return JSON responses. This behavior allows client programs which wish to communicate with a service to be written in any language.
...
- Hardware services expose the functionality of a connected hardware device (ADCS, GPS, radio, etc) to the rest of the bus. They should be re-usable between missions. KubOS comes with support for a certain selection of [pre-built hardware services](/docs/kubos/legacy-sphinx-home#supported-hardware).
...
Official Kubos Linux installation files can be found in the [kubos-linux-build releases](https://github.com/kubos/kubos-linux-build/releases) page.

Guides for installing and interacting with Kubos Linux on a target OBC can be found in the [../obc-docs/index](../obc-docs/index) section.

Information about creating a custom Kubos Linux build can be found in our [Under the Hood](/docs/kubos/deep-dive#custom-klb) section.

Tightly coupled with Kubos Linux is U-Boot, our bootloader of choice. U-Boot is responsible for loading Kubos Linux from permanent storage into RAM at boot time. It is also responsible for processing operating system [upgrades](linux-docs/kubos-linux-upgrade) and [recovery](linux-docs/kubos-linux-recovery), when necessary.

## Pages in This Section

- [apps/app-guide](apps/app-guide)
- [apps/python-app-api](apps/python-app-api)
- [apps/rust-app-api](apps/rust-app-api)
...
- [services/service-outline-guide](services/service-outline-guide)
