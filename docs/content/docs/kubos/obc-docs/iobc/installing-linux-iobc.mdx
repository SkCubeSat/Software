---
title: "Installing Kubos Linux on an ISIS-OBC"
---
...
- `working-with-the-iobc` - Guide for interacting with iOBC-specific features
- [../../deep-dive/klb/kubos-linux-on-iobc](../../deep-dive/klb/kubos-linux-on-iobc) - Steps to build Kubos Linux
...
Download the latest <span className="title-ref">KubOS-v\{version\}.tar.gz</span> file and then unzip the files for the iOBC. They're located in the <span className="title-ref">KubOS-v\{version\}/iOBC</span> folder.

## Install the SD Card Files

All users should install the SD card files using a distributed Kubos Linux image, unless they have created a custom Kubos Linux build. In that case, the SD card files can be installed by either flashing a complete Kubos Linux image onto an SD card or [by alternate means](/docs/kubos/obc-docs/iobc/installing-linux-iobc#alt-sd-setup).
...
This can be done by using the provided command line script or [using the SAM-BA GUI](/docs/kubos/obc-docs/iobc/installing-linux-iobc#alt-nor-setup).
...
> [!NOTE]
> Once Kubos Linux has been initially installed, future releases of the device tree can be installed using the [upgrade-installation](/docs/kubos/ecosystem/linux-docs/kubos-linux-upgrade#upgrade-installation) process with a <span className="title-ref">kpack-nor-\*.itb</span> file, rather than by manual flashing.

### Pre-Requisites

1.  Obtain an [Atmel SAM-ICE programmer/debugger](https://www.microchip.com/DevelopmentTools/ProductDetails/AT91SAM-ICE).
...
4.  Install SAM-BA from the ISIS-OBC SDK installer. (Refer to Section 3.3 of the <span className="title-ref">ISIS-OBC Quick Start Guide</span>)
...
```
5.  Setup the iOBC board for serial connection and programming. (Refer to Chapter 4 of the <span className="title-ref">ISIS-OBC Quick Start Guide</span>)
...
8.  Copy the <span className="title-ref">kubos-nor-flash.tcl</span> script from the <span className="title-ref">tools/at91sam9g20isis</span> folder in the [kubos-linux-build](https://github.com/kubos/kubos-linux-build) repo into the SAM-BA application folder.

9.  Change line 44 in <span className="title-ref">\{path to SAM-BA\}/tcl_lib/boards.tcl</span> from this:
...
You can do this via a Kubos SDK Vagrant image with the `minicom kubos` command after booting the board.
...
<figure>
<img src="/kubos/images/iOBC/uboot_console.png" alt="U-Boot Console" />
<figcaption aria-hidden="true">U-Boot Console</figcaption>
</figure>
...
For information on how to create and run applications on your new Kubos Linux system, see the `working-with-the-iobc` guide.
...
Since you are not using a pre-built Kubos Linux image, you will need to go through the [OS build process](/docs/kubos/deep-dive/klb/kubos-linux-on-iobc#build-os) locally in order to create the kernel and rootfs files.
...
8.  Obtain the NOR flash files either from Kubos, or from your own [local build](/docs/kubos/deep-dive/klb/kubos-linux-on-iobc#build-os):
...
<figure>
<img src="/kubos/images/iOBC/samba_connection_select.png" alt="SAM-BA Connection Selection" />
<figcaption aria-hidden="true">SAM-BA Connection Selection</figcaption>
</figure>
...
<figure>
<img src="/kubos/images/iOBC/samba_enable_norflash.png" alt="SAM-BA Enable NorFlash" />
<figcaption aria-hidden="true">SAM-BA Enable NorFlash</figcaption>
</figure>
...
<figure>
<img src="/kubos/images/iOBC/samba_send_uboot.png" alt="SAM-BA Send U-Boot" />
<figcaption aria-hidden="true">SAM-BA Send U-Boot</figcaption>
</figure>
...
<figure>
<img src="/kubos/images/iOBC/samba_send_dtb.png" alt="SAM-BA Send DTB" />
<figcaption aria-hidden="true">SAM-BA Send DTB</figcaption>
</figure>
