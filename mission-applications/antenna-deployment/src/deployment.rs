...
pub fn run_once() -> Result<(), Error> {
    ...
    if elapsed < HOLD_TIME_SECONDS {
        info!(
            "hold timer active: elapsed={}s remaining={}s",
            elapsed,
        ...
        match gpio::read_pin(GPIO_66) {
            Ok(true) => {
                ...
                if let Err(err) = state::set_flag(MissionFlagKey::VhfAntennaDeployed, true) {
                    ...
                    info!("VHF confirmed during pre-check refresh");
                }
            }
            Ok(false) => {}
        ...
        match gpio::read_pin(GPIO_7) {
            Ok(true) => {
                ...
                if let Err(err) = state::set_flag(MissionFlagKey::UhfAntennaDeployed, true) {
                    ...
                    info!("UHF confirmed during pre-check refresh");
                }
            }
            Ok(false) => {}
            Err(err) => warn!("UHF refresh read failed: {}", err),
        }
    }
}
        ...
        match gpio::read_pin(GPIO_66) {
            Ok(true) => {
                ...
                if let Err(err) = state::set_flag(MissionFlagKey::VhfAntennaDeployed, true) {
                    ...
                    info!("VHF confirmed during check-only cycle");
                }
            }
            Ok(false) => {}
        ...
        match gpio::read_pin(GPIO_7) {
            Ok(true) => {
                ...
                if let Err(err) = state::set_flag(MissionFlagKey::UhfAntennaDeployed, true) {
                    ...
                    info!("UHF confirmed during check-only cycle");
                }
            }
            Ok(false) => {}
            Err(err) => warn!("UHF check-only read failed: {}", err),
        }
    }
}
