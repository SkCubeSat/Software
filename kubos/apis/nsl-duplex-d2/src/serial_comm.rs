...
use radio_api::{Connection, RadioError, RadioResult, Stream};
use serial::prelude::*;
use std::io;
use std::io::prelude::*;
...
impl Stream for SerialStream {
    fn write(&self, data: &[u8]) -> RadioResult<()> {
        serial_send(&self.bus, data).map_err(|e| RadioError::Other(e.to_string()))
    }
    fn read(&self) -> RadioResult<Vec<u8>> {
        serial_receive(&self.bus).map_err(|e| RadioError::Other(e.to_string()))
    }
}
