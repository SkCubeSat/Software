...
impl Config {
    ...
    /// Get config data encoded as a hexadecimal string and convert it to a u8
    ///
    /// # Arguments
    /// * `key` - Key of value to get from config
    ///
    /// # Returns
    /// * `Some(u8)` - The hexadecimal value converted to u8 if found and valid
    /// * `None` - If the key doesn't exist or value cannot be parsed as hex
