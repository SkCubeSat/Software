...
pub fn read_ser(socket: &Arc<Mutex<SerialComms>>) -> Result<Vec<u8>, CommsServiceError> {
    if let Ok(socket) = socket.lock() {
        socket
            .read()
            .map_err(|e| CommsServiceError::GenericError(format!("{}", e)))
    } else {
        Err(CommsServiceError::MutexPoisoned)
    }
}

pub fn write_ser(socket: &Arc<Mutex<SerialComms>>, data: &[u8]) -> Result<(), CommsServiceError> {
    if let Ok(socket) = socket.lock() {
        socket
            .write(data)
            .map_err(|e| CommsServiceError::GenericError(format!("{}", e)))
