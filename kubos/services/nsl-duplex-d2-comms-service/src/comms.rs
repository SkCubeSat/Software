...
pub fn read(radio: &Arc<Mutex<DuplexComms>>) -> Result<Vec<u8>, CommsServiceError> {
    if let Ok(radio) = radio.lock() {
        radio
            .read()
            .map_err(|e| CommsServiceError::GenericError(e.to_string()))
    } else {
...
pub fn write(radio: &Arc<Mutex<DuplexComms>>, data: &[u8]) -> Result<(), CommsServiceError> {
    if let Ok(mut radio) = radio.lock() {
        radio
            .write(data)
            .map_err(|e| CommsServiceError::GenericError(e.to_string()))
